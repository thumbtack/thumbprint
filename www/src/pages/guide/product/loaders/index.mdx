...
Loading animations let the user know that content is on the way. The more consistent we are with the experience the more trust we’ll build with our users that the requested content will load shortly. Because we’re asking the user to wait, we want take steps to limit this delay and, where possible, preview the content that is coming.
...
Each of these are indeterminate loaders as time to completion is unknown. Deciding which one to use will require some judgement.
...
<Grid>
    ...
    <GridColumn aboveSmall={4}>
        ...
        <Text size={2} className="mt2 black-300">
            An action was triggered and we want to prevent further actions while we’re waiting for new content.
        </Text>
    </GridColumn>
    <GridColumn aboveSmall={4}>
