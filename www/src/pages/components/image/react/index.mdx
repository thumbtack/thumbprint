...
In some cases, like server-side rendering, you want the HTML to be complete without the need for client-side javascript. The `forceEarlyRender` prop overrides the default `Image` behavior that would both calculate the `sizes` attribute and lazy load the image.
