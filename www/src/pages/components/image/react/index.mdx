...
For rare situations where images that appear “above the fold” need to load instantly. This can be useful for server-side rendering.

```jsx
<Image
    src="https://d1vg1gqh4nkuns.cloudfront.net/i/363552040414527488/width/1024.jpeg"
    alt="Boxes with Thumbtack logos and phrases"
    disableLazyLoading
/>
```

## Manually setting a `size` value

The following image is intentionally blurry. In this case `forceSizes` overrides the default `Image` behavior that otherwise automatically calculates the `sizes` attribute for you. For this example `sizes` is set to `100px` which triggers a request for the `120w` webp or jpeg image, hence the blurry image.

This can be useful for server-side rendering, for example, on images that are `100vw`. See the section [Resolution switching: Different sizes](https://developer.mozilla.org/en-US/docs/Learn/HTML/Multimedia_and_embedding/Responsive_images#Resolution_switching_Different_sizes) for allowable values.

<Alert type="warning" title="Caution">
    By default the Image component calculates the sizes attribute for you. An incorrect manual value
    can negatively impact performance or image clarity. Use this prop only if necessary.
</Alert>
