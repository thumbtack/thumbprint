...
```jsx
<SmartImage id="350357374752325652" aspectRatio={1600 / 864} />
```
...
>
    {({ src: placeholderSrc, srcSet: innerSrcSet }) => (
        <ResponsiveImage srcSet={innerSrcSet}>
            {({ ref, sizes, src, srcSet, className }) => (
                <>
                    <picture ref={ref} className={className}>
                        {srcSet.map(s => (
                        ...
                        <img src={src ? src : placeholderSrc} sizes={sizes} alt="" />
                    </picture>
                    <p className="mt3">Image width: {sizes}</p>
                </>
            )}
