...
Images are always lazy-loaded. The `containerAspectRatio` prop is recommended to reserve space for the lazy-loaded image. This avoids [content reflow](https://css-tricks.com/preventing-content-reflow-from-lazy-loaded-images/) issue that causes the page to "jump" as the image gets loaded.
...
<div className="flex">
    <Image
        src="https://d1vg1gqh4nkuns.cloudfront.net/i/323483178133774346/width/1024.jpeg"
...
## Cropped images with `height` and `containerAspectRatio` props
...
This photo of a cat is cropped with the `containerAspectRatio` prop. By default, the image is `1024` x `640` pixels. The `objectPosition` controls how the image is aligned within the cropped container.

```jsx
<Image
    src="https://d1vg1gqh4nkuns.cloudfront.net/i/323475841931862023/width/1024.jpeg"
    alt="Cat laying on its back"
    containerAspectRatio={1.2 / 1}
