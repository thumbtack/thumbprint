...
Navigation bars for iOS are implemented using the built-in UIKit component `UINavigationBar`,
however they should be configured using the Thumbprint decorator method `NavigationBar.configure(_:appearance:content:)`
to apply one of the standard Thumbprint navigation bar styles.

### Appearance

There are three standard navigation bar appearances. In addition, custom themes can be defined by calling
the `NavigationBar.Appearance` constructor directly.
...
There are two standard content styles. The content style defines how foreground content such as text
should be displayed.
...
If set to `nil`, will default to the content style's foreground color. Set to `.some(nil)` to
disable this default and actually set the navigation bar's tint color to nil.
...
#### `public static let transparent: NavigationBar.Apperance`
...
export const pageQuery = graphql`
    {
        ...
        reactComponentProps: allFile(
            filter: { relativePath: { in: ["thumbprint-react/components/Alert/index.tsx"] } }
        ) {
