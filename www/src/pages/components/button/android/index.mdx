...
2. `app:isEnabled` in XML or `ThumbprintButton.isEnabled`, Boolean; defaults to true
3. `app:isLoading` in XML or `ThumbprintButton.isLoading`, Boolean; defaults to false
4. `app:isBleed` in XML or `ThumbprintButton.isBleed`, Boolean; defaults to false

Note: `app:buttonText` and `ThumbprintButton.buttonText` are deprecated. Use `android:text` and `ThumbprintButton.text` instead.
