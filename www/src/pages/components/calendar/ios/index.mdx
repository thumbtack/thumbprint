...
![CalendarPicker - Mutliple Selection](/img/ios/calendar_multiple_selection.png 'CalendarPicker - Multiple Selection')

In addition to being notified when the user interacts with the calendar's selected dates, the developer may query or set the calenddar's selected dates directly.
...
If the `calendarHeightMode` is set to `dyanmic`, then the calendar will expand and collapse as as necessary to display the minimum number of rows possible for each month. After transitioning to a new month, a `dynamic` height CalendarPicker will update its own internal height constraint and notify its delegates of the update.
