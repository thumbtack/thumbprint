...
To fix keep the height of the modal (including top row with close button and sticky footer) such that it remains consistent as modal content changes, you can apply `heightAboveSmall` prop. When used in conjunction with `<ModalFooter isSticky>`, while this sets a height above [the `small` breakpoint](https://thumbprint.design/guide/product/breakpoints/), the modal's height will not expand beyond the height of the viewport.
