...
If the `imageUrl` is provided but it doesn't point to a valid image, the `initials` will be displayed as a fallback.

```jsx
<>
    <UserAvatar imageUrl="https://example.com/404.jpg" initials="AA" size="large" />
    <EntityAvatar imageUrl="https://example.com/404.jpg" initials="B" size="large" />
</>
```
