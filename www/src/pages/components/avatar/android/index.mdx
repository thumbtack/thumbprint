...
<Alert type="warning" title="Mobile Components are not currently open source">
    We are still working on making our mobile components available for everyone to use. These docs
...
Note that image URLs and avatar initials are _not_ specified in the layout;
those are specified in the avatar's `.bind()` method.
...
This badge indicates that a user or entity is online. It can be set initially through the `.bind()` method,
or dynamically through the `setIsOnline()` method.
