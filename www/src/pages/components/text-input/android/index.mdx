...
There are four types of Text Input components:
...
-   `ThumbprintTextInputWithDrawables` : A single line `ThumbprintEditText` with drawables. Contains logic for capturing clicks on drawables (both start and end). Can expose drawable tap events through `.uiEvents()`.
-   `ThumbprintTextArea` : A multi-line ThumbprintEditText. For iOS and web parity, this is referred to as a TextArea.
