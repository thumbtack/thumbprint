...
type ImageFormat = 'image/webp' | 'image/jpeg' | 'image/png' | 'image/gif';
...
const Image = forwardRef<HTMLElement, PropTypes>(
    (
        {
            src,
            sources = [],
        ...
        const [
            ...
            setBrowserSupportIntersectionObserver,
        ] = useState(canUseDOM && typeof window.IntersectionObserver !== 'undefined');
        ...
        const objectFitProps: ObjectFitType = {};
        ...
        const webpSource = find(sources, (s): boolean => s.type === 'image/webp');
        const imgTagSource = find(
            sources,
            (s): boolean => s.type === 'image/jpeg' || s.type === 'image/png',
        );
