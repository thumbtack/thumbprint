...
test('`onCloseClick` is not called when non-ESC keys are pressed', () => {
    ...
    document.dispatchEvent(new KeyboardEvent('keydown', { keyCode: ENTER_KEY }));
    document.dispatchEvent(new KeyboardEvent('keydown', { keyCode: BACKSPACE_KEY }));
    document.dispatchEvent(new KeyboardEvent('keydown', { keyCode: SPACE_KEY }));
    document.dispatchEvent(new KeyboardEvent('keydown', { keyCode: LOWERCASE_A_KEY }));
...
describe('does not close modal on `ESC`', () => {
    ...
    test('when modal is closed and `shouldCloseOnEscape` is `false`', () => {
        const onCloseClick = jest.fn();
        mount(
            <ModalCurtain stage="exited" onCloseClick={onCloseClick} shouldCloseOnEscape={false} />,
        );
